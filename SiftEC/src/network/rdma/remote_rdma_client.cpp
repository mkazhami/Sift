...
uint64_t RemoteCoordinator::getRDMABufferBlock(size_t size, WorkRequestCommand type) {
    ...
    if (buffer_offset == (uint64_t)-1) DIE("Buffer offset cannot be -1");
...
int RemoteCoordinator::Read(WorkRequest * workReq) {
    if (state != RUNNING) {
        return -1;
    }
    ...
    struct connection * conn = (struct connection*)((WorkRequest *)c->conn->context)->getConnection(node_id);
    workReq->setConnection(node_id, conn);
...
int RemoteCoordinator::CompSwap(WorkRequest * workReq) {
    if (state != RUNNING) {
        return -1;
    }
...
int RemoteCoordinator::Write(WorkRequest * workReq) {
    if (state != RUNNING) {
        return -1;
    }
    ...
    rdma_write(workReq, workReq->getOffset(), workReq->getSize(), workReq->getValue()[node_id]);

    return 1;
}

int RemoteCoordinator::Write(std::vector<WorkRequest*> &workRequests) {
    if (state != RUNNING) {
        return -1;
    }
    ...
    for (int i = 0; i < workRequests.size(); i++) {
        ...
        workRequest->setConnection(node_id, (struct connection*)((WorkRequest *)c->conn->context)->getConnection(node_id));
        offsets[i] = workRequest->getOffset();
        sizes[i] = workRequest->getSize();
        buffers[i] = workRequest->getValue()[node_id];
    }
...
bool RemoteCoordinator::CreateConnection() {
    if (c->conn != nullptr && ((WorkRequest*)c->conn->context)->getConnection(node_id) != nullptr) {
...
void RemoteCoordinator::build_connection(struct rdma_cm_id *id, client *c) {
    ...
    workRequest->setConnection(node_id, conn);
...
void RemoteCoordinator::on_completion(struct ibv_wc *wc) {
    ...
    if (state != INIT && state != RECOVER) completed.fetch_add(1);
...
void RemoteCoordinator::destroy_connection(void *context) {
    struct connection *conn = (struct connection*)((WorkRequest*)context)->getConnection(node_id);
    ...
    free(conn);
    ((WorkRequest*)context)->setConnection(node_id, nullptr);
    c->conn = nullptr;
...
int RemoteCoordinator::rdma_write(const std::vector<WorkRequest *> &wrs, const std::vector<uint64_t> &offsets, const std::vector<size_t> &sizes, const std::vector<char*> &buffers) {
    ...
    while (ticket > completed && ticket + wrs.size() - completed > CQ_QUEUE_SIZE) {}

    int ret = ibv_post_send(conn->qp, &send_wrs[0], &bad_wr);
    if (ret != 0) {
        LogError("ibv_post_send failed");
        send_ticket.fetch_sub(wrs.size());
