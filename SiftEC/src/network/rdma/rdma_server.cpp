...
void MemoryServer::build_connection(struct rdma_cm_id *id, bool isAdmin) {
    ...
    workRequest->setConnection(0, conn);
...
void MemoryServer::register_memory(Server* server, WorkRequest *workRequest) {
    ...
    struct server_connection *conn = (struct server_connection*)workRequest->getConnection(0);
...
void MemoryServer::on_connect(void *context) {
    ...
    ((struct server_connection*)workRequest->getConnection(0))->connected = 1;
...
void MemoryServer::on_completion(struct ibv_wc *wc){
    ...
    struct server_connection *conn = (struct server_connection*)workRequest->getConnection(0);//(struct connection *)(uintptr_t)wc->wr_id;
