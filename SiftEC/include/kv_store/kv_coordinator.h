...
private:
    ...
    // TODO: make write-ahead log blocks match up with the size of the replicated memory's blocks

    static const uint64_t COMMITTED_INDEX_OFFSET = 0;
    static const uint64_t APPLIED_INDEX_OFFSET = COMMITTED_INDEX_OFFSET + COMMITTED_INDEX_SIZE + RM_MEMORY_BLOCK_SIZE - ((COMMITTED_INDEX_OFFSET+COMMITTED_INDEX_SIZE)%RM_MEMORY_BLOCK_SIZE);
    static const uint64_t INDEX_TABLE_OFFSET = APPLIED_INDEX_OFFSET + APPLIED_INDEX_SIZE + RM_MEMORY_BLOCK_SIZE - ((APPLIED_INDEX_OFFSET+APPLIED_INDEX_SIZE)%RM_MEMORY_BLOCK_SIZE);
    static const uint64_t DATA_TABLE_OFFSET = INDEX_TABLE_OFFSET + INDEX_TABLE_SIZE + RM_MEMORY_BLOCK_SIZE - ((INDEX_TABLE_OFFSET+INDEX_TABLE_SIZE)%RM_MEMORY_BLOCK_SIZE);
    static const uint64_t BITMAP_OFFSET = DATA_TABLE_OFFSET + DATA_TABLE_SIZE + RM_MEMORY_BLOCK_SIZE - ((DATA_TABLE_OFFSET+DATA_TABLE_SIZE)%RM_MEMORY_BLOCK_SIZE);
    static const uint64_t WRITE_AHEAD_LOG_OFFSET = BITMAP_OFFSET + BITMAP_SIZE + RM_MEMORY_BLOCK_SIZE - ((BITMAP_OFFSET+BITMAP_SIZE)%RM_MEMORY_BLOCK_SIZE);

    LocalReplicationServer *replicationServer;

    //Queue<std::pair<std::shared_ptr<RequestProcessor>, KVRequest*>> logWriteFutureQueue;
