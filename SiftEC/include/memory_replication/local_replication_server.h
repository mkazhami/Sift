...
public:
    ...
    LocalReplicationServer(const uint32_t &serverId) {
        coordinatorServer = new CoordinatorAdmin(CONFIG_FILE, serverId);
        coordinatorService = new CoordinatorService(serverId);
    ...
    std::shared_ptr<RequestProcessor> readRequest(uint64_t address) {
