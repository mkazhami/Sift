...
CoordinatorAdmin::CoordinatorAdmin(std::string configFile, int node_id, PersistentStore* persistentStore)
        : node_id(node_id), startBarrier(TOTAL_MEMORY_SERVERS+1), endBarrier(TOTAL_MEMORY_SERVERS+1), terminate(false), persistentStore(persistentStore) {
        ...
        try {
            if (i == LOCAL_MEMORY_NODE && COORDINATOR_MEMORY_NODE) {
                ...
                if (rdma_clients[i]->CreateConnection()) {
                    if (USE_PERSISTENCE) {
                        // Nothing to recover at this point
                        rc->post_send();
                        rc->post_recv();

                        // Wait for recovery messages to be exchanged
                        while (rdma_clients[i]->state != RUNNING) {}
                    }
    ...
    while (true) {
        ...
        for (int i = 0; i < numBlocksToCopy; i++) {
            ...
            if (coordinatorService->usedBlocks.getBit(lockedBlockIndex + i) != 0) {
                ...
                rdma_clients[upToDateServer]->Read(workRequest, true);
        ...
        for (int i = 0; i < numBlocksToCopy; i++) {
            ...
            if (workRequest != nullptr) {
                ...
                rdma_clients[node]->Write(workRequest, true);
                requests[i] = workRequest;
...
void CoordinatorAdmin::readHeartBeat(int i) {
    ...
    char *data = workRequest->getValue();
...
void CoordinatorAdmin::writeLogEntry(int server, uint32_t idx, char *log_entry) {
    ...
    WorkRequest *workRequest = new WorkRequest;
    workRequest->setValue(log_entry);
...
void CoordinatorAdmin::updateAppliedIndex(int server, uint32_t new_index) {
    ...
    WorkRequest *workRequest = new WorkRequest;
    workRequest->setValue((char*)&new_index);
...
void CoordinatorAdmin::readLog(int server, std::vector<SerializableAVPair::AVPair*>& log, Barrier *barrier) {
    ...
    while (read_entries < RM_LOG_SIZE) {
        ...
        for (int i = 0; i < entries_to_read; i++) {
            ...
            SerializableAVPair::AVPair *log_entry = (SerializableAVPair::AVPair*)workRequest->getValue();
