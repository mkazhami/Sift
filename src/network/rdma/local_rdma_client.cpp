...
int LocalCoordinator::Read(WorkRequest* workReq, bool force) {
    ...
    workReq->copyValue(data + address);
...
int LocalCoordinator::Write(WorkRequest* workReq, bool force) {
    uint64_t address = workReq->getOffset();
    uint64_t size = workReq->getSize();
    char *value = workReq->getValue();
...
int LocalCoordinator::Write(std::vector<WorkRequest*> &workReqs, bool force) {
...
int LocalCoordinator::CompSwap(WorkRequest* workReq, bool force) {
