...
void WorkRequest::copyValue(char *value) {
    this->value = new char[size];
    std::memcpy(this->value, value, size);
    valueReady.store(true);
}

bool WorkRequest::ready() {
    return valueReady.load();
}
...
void WorkRequest::setValue(char *value) {
    this->value = value;
}
...
void *WorkRequest::getConnection() {
    return conn;
}

void WorkRequest::setConnection(void *conn) {
    this->conn = conn;
}
