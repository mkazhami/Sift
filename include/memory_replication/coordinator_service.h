...
public:
    CoordinatorService(int nodeId, PersistentStore*);
    ~CoordinatorService();

    // Initializes clients and starts apply thread
    void init(Coordinator *clients[]);
    // Takes completed log writes and submits the state machine update
    void apply();
    void applyWriteRequest(WorkRequest*);

    // API to submit requests
    ...
    // Callbacks for completed RDMA requests
    void processLogWriteCompletion(WorkRequest*, int, int);
    void processWriteCompletion(WorkRequest*, int, int);
    void processReadCompletion(WorkRequest*);
    // Failed RDMA request callback
...
protected:
    ...
    // Queue for completed log writes
    Queue<WorkRequest *> applyQueue;

    // Variables tracking log state
    ...
    WeightedBoundedQueue<std::pair<uint64_t, std::shared_ptr<RequestProcessor>>, RM_LOG_SIZE> persistentLogQueue;
